...
For starters, it can throw a bunch of exceptions. I've said it probably too many times, but I'll say it again: exceptions shouldn't be used for non-exceptional cases. Invalid inputs isn't exceptional; not found entities isn't exceptional; breaking a domain rule isn't exceptional. Not only are we using a mechanism that should be used only when things go terribly wrong, we're hiding the behavior of our components, because it's not part of the functions signature, so we don't even know that they might happen (and even if they were part of the signature, like in Java, things would only get messier, not less).
